...
def f(for_seed, n, upper_bound):
    ...
    # duplicates = set()
    ordered_square = [[0 for _ in range (n)] for _ in range(n)]
    ...
    # print(arr)
    duplicates = sorted(set([x for x in arr if arr.count(x) > 1]))
    # print(duplicates)
    output = ''
    if len(duplicates):
        for dup in duplicates:
            output += str(dup) + ' '
        print(f'It is not a good square because it contains duplicates, namely: {output[:-1]}')
    else:
        ctr = 0
        sort_arr = sorted(arr)
        for i in range(len(square)):
            for j in range(len(square)):
                ordered_square[j][i] = sort_arr[ctr]
                ctr += 1
